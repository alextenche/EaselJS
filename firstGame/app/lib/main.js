...
function tryMove (obj, x, y){
	...
	if (tryTileTop.name == 'block') return false; // hitted a block

	if (tryTileTop.name == 'box') { // hitted a box
		if(obj.name == 'box') return false // can't push two boxes
		var canTryTileMove = tryMove(tryTileTop, x, y); // try pushing a box
		if(!canTryTileMove) return false // quick exit if box cant be moved
	}
